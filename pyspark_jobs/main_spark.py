...
def read_source_to_bronze(
    spark: SparkSession, input_path: str, warehouse_path: str, config: dict
):
    ...
    df.write.format("delta").mode("overwrite").partitionBy("invoice_date_date").save(
        output_bronze_path
    )
    # for profiling data in bigquery for dev
    labels = config.get("table_labels", {}).get("bronze", {})
    upload_to_bigquery(
        spark,
        df,
        f"{project_id}.{dataset_bronze}.{table_bronze}",
        config["buckets"]["temp"],
        labels=labels,
    )
...
def cleanse_and_load_to_silver(
    spark: SparkSession, bronze_path: str, warehouse_path: str, config: dict
):
    ...
    path_dim_customer = (
        f"{warehouse_path}/{config['paths']['silver_tables']['dim_customer']}"
    )
    start_cust_key = get_next_surrogate_key_start(
        spark, path_dim_customer, "customer_key"
    )
    ...
    df_dim_customer = (
        ...
        .withColumn(
            "rank",
            row_number().over(
                Window.partitionBy("customer_id").orderBy(col("ingesttion_time").desc())
            ),
        )
    ...
    df_dim_customer = df_dim_customer.withColumn(
        "customer_key",
        (
            row_number().over(Window.orderBy("customer_id")) + lit(start_cust_key) - 1
        ).cast("int"),
    ).select(col("customer_key"), col("customer_id"), col("country_name"))

    # 2. Product Dimension
    path_dim_product = (
        f"{warehouse_path}/{config['paths']['silver_tables']['dim_product']}"
    )
    start_prod_key = get_next_surrogate_key_start(
        spark, path_dim_product, "product_key"
    )
    ...
    df_dim_product = (
        ...
        .withColumn(
            "rank",
            row_number().over(
                Window.partitionBy("stock_code").orderBy(col("ingesttion_time").desc())
            ),
        )
    ...
    df_dim_product = df_dim_product.withColumn(
        "product_key",
        (
            row_number().over(Window.orderBy("stock_code")) + lit(start_prod_key) - 1
        ).cast("int"),
    ).select(
        col("product_key"),
        col("stock_code"),
    ...
    # 3. Country Dimension
    path_dim_country = (
        f"{warehouse_path}/{config['paths']['silver_tables']['dim_country']}"
    )
    start_country_key = get_next_surrogate_key_start(
        spark, path_dim_country, "country_key"
    )
    ...
    df_dim_country = (
        ...
        .withColumn(
            "rank",
            row_number().over(
                Window.partitionBy("country").orderBy(col("ingesttion_time").desc())
            ),
        )
    ...
    df_dim_country = df_dim_country.withColumn(
        "country_key",
        (
            row_number().over(Window.orderBy("country")) + lit(start_country_key) - 1
        ).cast("int"),
    ...
    df_dim_date = (
        ...
        .withColumn(
            "rank",
            row_number().over(
                Window.partitionBy("date").orderBy(col("ingesttion_time").desc())
            ),
        )
        .where(col("rank") == 1)
        .drop("rank", "ingesttion_time")
    ...
    df_dim_date = df_dim_date.withColumn(
        "date_key",
        (row_number().over(Window.orderBy("date")) + lit(start_date_key) - 1).cast(
            "int"
        ),
    ).select(
        col("date_key"),
    ...
    df_fact = df.withColumn("date", to_date(col("invoice_date"))).where(
        ~col("invoice").startswith("C")
    )  # filter out cancelled transaction
    df_fact = (
        df_fact.alias("fact")
        .join(
            broadcast(df_dim_customer.alias("cust")),
            col("fact.customer_id") == col("cust.customer_id"),
            "left",
        )
        .join(
            broadcast(df_dim_product.alias("prod")),
            col("fact.stock_code") == col("prod.stock_code"),
            "left",
        )
        .join(
            broadcast(df_dim_country.alias("cntry")),
            col("fact.country") == col("cntry.country_name"),
            "left",
        )
        .join(
            broadcast(df_dim_date.alias("dt")),
            col("fact.date") == col("dt.date"),
            "left",
        )
    )
    ...
    df_transaction = (
        df_fact.withColumn(
            "transaction_key", row_number().over(Window.orderBy("invoice"))
        )
    ...
    # 1. Customer Dimension
    df_dim_customer_staging = (
        df_dim_customer.withColumn("is_current", lit(True))
    ...
    upload_to_bigquery(
        spark,
        df_current_customer,
        f"{project_id}.{dataset_datawarehouse}.dim_customer",
        temp_bucket,
        labels=labels,
    )
    ...
    # 2. Country Dimension
    df_dim_country_staging = (
        df_dim_country.withColumn("is_current", lit(True))
        .withColumn("start_date", current_timestamp())
        .withColumn("end_date", lit(None).cast(TimestampType()))
    )
    ...
    upload_to_bigquery(
        spark,
        df_current_country,
        f"{project_id}.{dataset_datawarehouse}.dim_country",
        temp_bucket,
        labels=labels,
    )
    ...
    df_dim_date_staging = (
        df_dim_date.withColumn("is_current", lit(True))
        .withColumn("start_date", current_timestamp())
        .withColumn("end_date", lit(None).cast(TimestampType()))
    )
    upsert_scd_type_2(
        spark, df_dim_date_staging, path_dim_date, "date_key"
    )  # date_key is surrogate, ideally use 'date'
    df_current_date = spark.read.format("delta").load(path_dim_date)
    labels = config.get("table_labels", {}).get("dim_date", {})
    upload_to_bigquery(
        spark,
        df_current_date,
        f"{project_id}.{dataset_datawarehouse}.dim_date",
        temp_bucket,
        labels=labels,
    )
    ...
    # 4. Product Dimension
    df_dim_product_staging = (
        df_dim_product.withColumn("is_current", lit(True))
        .withColumn("start_date", current_timestamp())
        .withColumn("end_date", lit(None).cast(TimestampType()))
    )
    ...
    upload_to_bigquery(
        spark,
        df_current_product,
        f"{project_id}.{dataset_datawarehouse}.dim_product",
        temp_bucket,
        labels=labels,
    )
    ...
    path_fact_transaction = (
        f"{warehouse_path}/{config['paths']['silver_tables']['fact_transaction']}"
    )
    df_transaction.write.format("delta").mode("overwrite").partitionBy(
        "invoice_date_date"
    ).save(path_fact_transaction)
    df_current_fact = spark.read.format("delta").load(path_fact_transaction)
    labels = config.get("table_labels", {}).get("fact_transaction", {})
    upload_to_bigquery(
        spark,
        df_current_fact,
        f"{project_id}.{dataset_datawarehouse}.fact_transaction",
        temp_bucket,
        labels=labels,
    )
...
def aggregate_and_load_to_gold(
    spark: SparkSession,
    warehouse_path: str,
    path_fact: str,
    path_cust: str,
    ...
    df_revenue_monthly = (
        df_fact.join(
            df_dim_date, df_fact["date_key"] == df_dim_date["date_key"], "left"
        )
        .groupBy("year", "month")
    ...
    path_revenue_monthly = (
        f"{warehouse_path}/{config['paths']['gold_tables']['revenue_monthly']}"
    )
    logger.debug(f"path_revenue_monthly : {path_revenue_monthly}")
    df_revenue_monthly.write.format("delta").mode("overwrite").save(
        path_revenue_monthly
    )
    ...
    df_product_revenue_performance = (
        df_fact.alias("fact")
        .join(
            df_dim_product.alias("dim_product"),
            df_fact["product_key"] == df_dim_product["product_key"],
            "left",
        )
    ...
    logger.debug(
        f"path_product_revenue_performance : {path_product_revenue_performance}"
    )
    df_product_revenue_performance.write.format("delta").mode("overwrite").save(
        path_product_revenue_performance
    )
    ...
    df_revenue_country = (
        df_fact.alias("fact")
        .join(
            df_dim_country.alias("dim_country"),
            df_fact["country_key"] == df_dim_country["country_key"],
            "left",
        )
    ...
    path_revenue_country = (
        f"{warehouse_path}/{config['paths']['gold_tables']['revenue_country']}"
    )
    df_revenue_country.write.format("delta").mode("overwrite").save(
        path_revenue_country
    )
    ...
    df_rfm = df_rfm_base.groupBy("c.customer_id").agg(
    ...
    df_rfm = (
        df_rfm.withColumn("R", ntile(5).over(Window.orderBy(col("Recency").asc())))
    ...
    df_rfm = df_rfm.withColumn(
        ...
        .when(
            (col("R") >= 3) & (col("F") >= 2) & (col("F") <= 3) & (col("M") >= 2),
            "Potential Loyalists",
        )
    ...
    for table_name, path in gold_tables.items():
        ...
        upload_to_bigquery(
            spark,
            df_gold,
            f"{project_id}.{dataset_datamart}.{table_name}",
            temp_bucket,
            labels=labels,
        )
...
def main(input_path: str, warehouse_path: str, project_id: str, config_file: str):
    ...
    (
        ...
        path_dim_date,
        path_dim_product,
    ) = cleanse_and_load_to_silver(spark, bronze_path, warehouse_path, config)

    # Gold Layer logic
    aggregate_and_load_to_gold(
        spark,
        warehouse_path,
        path_fact_transaction,
        path_dim_customer,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-i", "--input", required=True, help="Input GCS path for raw data"
    )
    parser.add_argument(
        "-o", "--output", required=True, help="Output GCS path for processed data"
    )
